...
def build_evidence(candidate: CandidateResult, context: str) -> list[EvidenceItem]:
    ...
    if source == "canonical_match":
        ...
        is_semantic_vector = source == "semantic_match" or "semantic_vector_retrieval" in candidate.reasons
        items.append(
            EvidenceItem(
                ...
                detail=(
                    ...
                    if source == "descriptive_match"
                        ...
                        f"语义向量召回：{candidate.matched_name}，score={candidate.score}"
                        if is_semantic_vector
    ...
    for reason in candidate.reasons:
        if reason == "alias_prior_support":
            items.append(
                EvidenceItem(
                    evidence_type=EvidenceType.model_inference,
                    detail="模型/先验加权依据：alias_prior_support",
                )
            )
        ...
        elif reason == "fuzzy_context_validated":
            items.append(
                EvidenceItem(
                    evidence_type=EvidenceType.model_inference,
                    detail="模糊召回结果已通过上下文验证",
                )
            )
    return items
